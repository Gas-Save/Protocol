...
contract("GSVE Contract Deployer Test", async accounts => {
    ...
    it('should be able to deploy a wrapper contract', async () => {
      await token.approve(factory.address, web3.utils.toWei("10"));
    ...
    it('should revert when trying to create a wrapper without tokens to pay pay', async () => {
      expectRevert(factory.deployGSVESmartWrapper({from:accounts[1]}), "ERC20: burn amount exceeds balance.");
    });
